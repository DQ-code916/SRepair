...
class Probabilistic():
    ...
    def Pos2(self,):     
        self.Lij={i:{j: self.Lij[i][j] for j in self.Lij[i].keys()} for i in self.Lij}
        self.Li={i:sum(find_top_k(self.Lij[i].values(), self.k_L, Type='smallest')[0]) for i in self.Lij.keys()}
