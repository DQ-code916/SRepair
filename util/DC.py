...
class DC():
    ...
    def check_cf_num(self, db, true_errors, if_CheckMem=False, if_process=False, max_workers=None):
        res = set()
        true_errors=list(range(len(db)))
        ...
        temp=len(true_errors)
        chunk_size = temp // max_workers
        remainder = temp % max_workers
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            futures = []
            for chunk in chunks:
                futures.append(executor.submit(process_chunk, chunk, D1, D2_fd, D2_dc, D2_0, D3, db, fDict, if_process))

            for future in futures:
                chunk_result = future.result()
                res.update(chunk_result)
...
def process_chunk(chunk, D1, D2_fd, D2_dc, D2_0, D3, db, fDict, if_process=False):
    res = set()
    n, m = db.shape
    total = len(chunk)
    t1 = time()
    ...
    for idx, i in enumerate(chunk):
        if if_process and idx % 500 == 0:
            t2 = time()
            elapsed_time = round(t2 - t1, 3)
            progress = (idx / total) * 100
            print(f"Process Chunk Progress: {progress:.2f}% - Currently Processing i={i} - Time Elapsed: {elapsed_time}s")
