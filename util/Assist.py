...
def TrainDModel_byHand_GPR(X, Y, sigma=1.0, alpha=1e-5, k=10):
    """
    Train Gaussian Process Regression model
    X: Independent variable matrix, shape (k, m), where k is number of samples, m is number of features
    Y: Dependent variable matrix, shape (k,)
    sigma: Hyperparameter of kernel function (controls smoothness)
    alpha: Regularization term added to covariance matrix
    k: Number of samples
    Returns: Trained phi and prediction variance
    """
    
    K = rbf_kernel(X, X, sigma)
    K += alpha * np.eye(k)
    phi = np.linalg.inv(K) @ Y
    
    e = np.zeros(k)
    for i in range(k):
        K_star = rbf_kernel(X[i:i+1, :], X, sigma)
        K_star_star = rbf_kernel(X[i:i+1, :], X[i:i+1, :], sigma)
        y_pred = K_star @ np.linalg.inv(K) @ Y
        y_var = K_star_star - K_star @ np.linalg.inv(K) @ K_star.T
        e[i] = abs(Y[i] - y_pred)
...
def rbf_kernel(X1, X2, sigma=1.0):
    """Compute RBF kernel function for two input matrices"""
    sqdist = np.sum(X1**2, axis=1).reshape(-1, 1) + np.sum(X2**2, axis=1) - 2 * np.dot(X1, X2.T)
    return np.exp(-0.5 * sqdist / sigma**2)


def predict_by_GPR(X_new, X_train, Y_train, phi, sigma=1.0, alpha=1e-5):
    """
    Predict using Gaussian Process Regression model and compute prediction error
    X_new: New independent variable vector, shape (1, m)
    X_train: Training set independent variables, shape (k, m)
    Y_train: Training set dependent variables, shape (k,)
    phi: Gaussian Process Regression coefficients, shape (k,)
    sigma: Hyperparameter of Gaussian kernel function
    alpha: Regularization term to prevent covariance matrix singularity
    Returns: Prediction error (difference between predicted and true values)
    """
    
    K = rbf_kernel(X_train, X_train, sigma)
    K += alpha * np.eye(len(X_train))
    
    K_star = rbf_kernel(X_new, X_train, sigma)
    K_star_star = rbf_kernel(X_new, X_new, sigma)
    
    y_pred = K_star @ np.linalg.inv(K) @ Y_train
    y_var = K_star_star - K_star @ np.linalg.inv(K) @ K_star.T
    
    true_value = Y_train[0]
    pred_error = abs(true_value - y_pred)
    
    return pred_error, y_pred, y_var
...
def Save_IN(ls,Method,pct=None):
    ls.IN=sorted(ls.IN)
    Save_IN_inner(ls.IN, ls.DataSet, Method, pct)
    


def Save_IN_inner(IN, Dataset, Method, pct=None):
    if pct is None:
        dir_path = f'./data/{Dataset}/IN_results'
        file_path = f'{dir_path}/{Method}_IN.txt'
    else:
        dir_path = f'./data/{Dataset}/{pct}%/IN_results'
        file_path = f'{dir_path}/{Method}_IN.txt'
...
def Read_IN(filepath):
    """
    Read IN.txt file
    Each line contains one integer, returns list of int
    """
    with open(filepath, 'r', encoding='utf-8') as f:
        return [int(line.strip()) for line in f if line.strip()]
    
def res_to_df(res: dict) -> pd.DataFrame:
